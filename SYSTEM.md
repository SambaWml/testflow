...
| Papel | Escopo | Resumo |
|-------|--------|--------|
| **Super Admin** | Global | Bypass total de org; acesso ao `/admin`; cria/edita/deleta qualquer org |
| **Owner** | Organização | Todos os projetos da org + configurações exclusivas (features, role-names, reset de senha) |
| **Admin** | Organização | Todos os projetos da org; convida/remove membros; cria projetos |
| **Member** | Projetos vinculados | Só projetos em `ProjectMember`; vê apenas seus próprios bugs |

### Matrix de permissões por rota

| Rota | Super Admin | Owner | Admin | Member |
|------|:-----------:|:-----:|:-----:|:------:|
| **Projetos** | | | | |
| `GET /api/projects` | todas as orgs | todos da org | todos da org | só os vinculados |
...
| **Membros da org** | | | | |
| `GET /api/orgs/members` | — | ✅ | ✅ | ✅ (só leitura) |
| `POST /api/orgs/members` (convidar) | — | ✅ | ✅ | ❌ 403 |
| `PATCH /api/orgs/members/[id]` | — | ✅ | ✅ (não pode editar OWNER) | ❌ 403 |
| `DELETE /api/orgs/members/[id]` | — | ✅ | ✅ (não pode remover OWNER) | ❌ 403 |
| `POST /api/orgs/members/[id]/reset-password` | — | ✅ | ❌ 403 | ❌ 403 |
| **Membros de projeto** | | | | |
| `GET /api/orgs/projects/[id]/members` | ✅ | ✅ | ✅ | ✅ |
| `POST /api/orgs/projects/[id]/members` | ✅ | ✅ | ✅ | ❌ 403 |
| `DELETE /api/orgs/projects/[id]/members` | ✅ | ✅ | ✅ | ❌ 403 |
| **Configurações da org** | | | | |
| `PATCH /api/orgs/features` | ✅ | ✅ | ❌ 403 | ❌ 403 |
| `PATCH /api/orgs/role-names` | ✅ | ✅ | ❌ 403 | ❌ 403 |
| **Casos de teste** | | | | |
| `GET /api/cases` | todos | todos da org | todos da org | só proj. vinculados |
| `POST /api/cases` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| `GET/PATCH/DELETE /api/cases/[id]` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| **Itens (User Stories)** | | | | |
| `GET /api/items` | todos | todos da org | todos da org | só proj. vinculados |
| `POST /api/items` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| `GET/PATCH/DELETE /api/items/[id]` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| **Execuções** | | | | |
| `GET /api/executions` | todos | todos da org | todos da org | só proj. vinculados |
| `POST /api/executions` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| `GET/PATCH /api/executions/[id]` | ✅ | ✅ | ✅ | ✅ (proj. vinculado) |
| **Planos de teste** | | | | |
| `GET /api/test-plans` | todos | todos da org | todos da org | só proj. vinculados |
...
| **Reports** | | | | |
| `GET /api/reports` | todos | todos da org | todos da org | só proj. vinculados |
...
- **MEMBER e bugs**: vê apenas os que ele mesmo criou (`authorId === u.id`)
- **OWNER não pode ser removido** via `/api/orgs/members` — apenas Super Admin via `/api/admin/`
- **Reset de senha** é exclusivo do OWNER — nem ADMIN pode redefinir senha de outros membros
- **Features e role-names** só o OWNER altera — impacto em toda a org
- **Super Admin** opera acima da estrutura de orgs — nunca aparece como membro de nenhuma org

### `src/lib/permissions.ts`

```typescript
// Retorna null (sem filtro) se OWNER/ADMIN, ou array de IDs dos projetos vinculados se MEMBER
...
Recursos de outra org retornam **404** (não 403) — nunca confirmar existência:
